...
create policy if not exists "measurement_photos_select"
  on public.measurement_photos for select
  to authenticated
  using (true);

create policy if not exists "measurement_photos_insert"
  on public.measurement_photos for insert
  to authenticated
  with check (true);

create policy if not exists "measurement_photos_delete"
...
create policy if not exists "Public read access to measurement-photos"
on storage.objects for select
to public
using (bucket_id = 'measurement-photos');

create policy if not exists "Authenticated can upload measurement-photos"
on storage.objects for insert
to authenticated
with check (bucket_id = 'measurement-photos');

create policy if not exists "Authenticated can delete own measurement-photos"
